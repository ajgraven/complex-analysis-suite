...
- **Execution: PHASE 0 IS MERGED, and PHASE 1 HAS BEGUN.**
  ...
  **PHASE 2 HAS BEGUN. Step 2.1 is done** — the shell's own prose, the three label maps in
  `engine/vocabulary.ts`, and the denylist, which is two tests because one instrument cannot see both
  halves. It found a crash older than M8 and two layouts that had never worked.
  **Next execution action: step 2.2** (record-level prose on screen).
