...
- **(3.2) A sweep outlived everything, and one path leaked a loop plus a permanent draft budget.**
  ...
  nothing on screen to say why. Press Play, then Ctrl+Z or open a `#vs=` link. **⚠ This was FOUND
  STILL LIVE at the 2026-09-20 review and is fixed in the remediation** (REPORT §4.1): `endSweep()`
  is called from `commit` only when the argument MOVED, and Ctrl+Z, a `#vs=` link, a contrast cell, a
  drill rung and `destroy()` can all land on the same record, fixture, mode and expression — so the
  test below it drove `stepOnce: true`, the one branch that cannot have the defect, and the rAF was
  measured still pending every frame after `destroy()`, the closure retaining the whole shell.
  Without a
...
## Post-M8 review (2026-09-20)

A full-app review of `apps/contour-integration` at `master` = `5ebfa54` — ten reviewers on disjoint
slices (the exact kernel, bounds and branch, engine and contour model, families and all 28 records,
the shell's state layer, the shell's cards, stage/GPU with the browser suite, a live click-through in
headless Chromium, the documentation, and the test suite), each under a read-only brief that
preferred a measurement to a reading. The mathematics came back sound — all 28 closed forms and all
94 fixture values re-derived from scratch and agreeing to 1e-12 or better — and the defects are in
the GATING of what may be shown, nine of them instances of one gap. The consolidated report is
[`../../review/2026-09-20-contour-integration-review/REPORT.md`](../../review/2026-09-20-contour-integration-review/REPORT.md),
its ten slice reports with every reproduction are beside it under `slices/`, and the work packages
that execute it are
