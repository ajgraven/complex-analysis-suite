...
- **Execution: PHASE 0 IS MERGED, and PHASE 1 HAS BEGUN.**
  ...
  **PHASE 2 HAS BEGUN. Steps 2.1, 2.2 and 2.3 are done** — the shell's own prose with its three label
  maps in `engine/vocabulary.ts` and the two-part denylist; then the ninety-four method strings and
  the Target card that shows them; then the figure export's three plates. Between them they found a
  crash older than M8, two layouts that had never worked, and an identity printed half in numbers and
  half in letters.
  **Next execution action: step 2.4** (errors, refusals and empty states in plain language).
