...
| The contrast and drill panels, `position: fixed` overlays appended to `<main>` | the drill is a rail card; the ladder is a centred modal mounted on `root` | §2.15. |
...
| No teardown — the `resize` listener is never removed and `GLStage.dispose` is never called | `destroy()` takes the key listener, the hash timer, the resize observer, the controller and all four views | `shell2.test.ts` › `LETS GO of the document when the shell is destroyed`; `strip.test.ts` › `is removed by destroy, along with the panel it built`; `contrasts.test.ts` › `can be closed before it was ever opened, and destroyed after`. `stageView.destroy` calls `stage.dispose()`, which the old shell had and never used. |
...
- **The three redundant pole listings — two of them, not three.** §2.8: the Result card's list is
  ...
  engine put it.
- **The full-screen overlays — one of them.** §2.15 and §1's panels row: the drill's overlay is a rail
  card, and the ladder's is a centred modal dialog (`max-width: min(92vw, 68rem)`, `max-height: 88vh`)
  with the mechanics the review found missing.
