...
| Aberth–Ehrlich solver + worker pool                                            | `apps/polynomial-roots/src/engine/roots/` (app-local by ADR-0046 decision 6, "until a second consumer appears") | **decided at PRA-1: moved to `@cas/core`** — DK diverged on Wilkinson 20 (STATUS, PRA-1.2) | second consumer, by measurement |
...
`animate.ts` → `@cas/ui`.
_(As executed, 2026-09-24: Aberth + exact refinement replaced DK + polish, the round-trip clause is
restated as what is true of an ill-conditioned polynomial, KaTeX and the `animate.ts` lift moved to the
milestones that first need them — each with its reason in STATUS.)_
