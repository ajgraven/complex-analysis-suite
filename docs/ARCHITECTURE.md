...
> The suite now ships **fourteen** packages: `@cas/core`, `@cas/interchange`, `@cas/expr`, `@cas/gpu`,
> `@cas/exact`, `@cas/schwarz`, `@cas/dynamics`, `@cas/export`, `@cas/conformal`, `@cas/faber`, `@cas/ui`, `@cas/flow`, `@cas/rigor` and `@cas/monodromy` (the last extracted from the plotter's `src/riemann/` at ADR-0047's PRA-3; `@cas/exact`
...
- **Publishing is automated.** `.github/workflows/deploy-pages.yml` runs on every push to `master`
  ...
  **one combined Pages site**: `apps/launcher/dist` at the root, with `complex-dynamics/`,
  ...
  (`apps/correspondences` is built but **not** published; Polynomial Root Analysis joined at its PRA-5,
  ADR-0047). Note the
...
The launcher is a static stub (`apps/launcher`) listing all thirteen published apps plus one "Coming soon"
card — Correspondences (fourteen cards in all), and it is the **whole** of the unified menu: the in-app nav header
