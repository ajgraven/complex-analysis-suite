...
| 18 ✅ | Shape-from-moments (Prony–Hankel) reconstruction + QD-order rank test — `hankelRank` (QD-order = exact ℚ(i) Hankel rank drop) + `pronyPolynomial` (exact Prony poly Π(z−z_j)) + `shapeFromMoments` (numeric nodes via Durand–Kerner on the exact Prony poly + Vandermonde weights + reconstruction residual). QD wire/UI deferred (engine-only, like #12–#15) | ★★ | Moderate | zero-dim root-find, exact null-space/Vandermonde |
