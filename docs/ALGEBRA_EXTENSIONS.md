...
| 18 ◐ | Shape-from-moments (Prony–Hankel) reconstruction + QD-order rank test — exact core `hankelRank` (QD-order = exact ℚ(i) Hankel rank drop) + `pronyPolynomial` (exact Prony poly Π(z−z_j)) ✅; full `shapeFromMoments` (numeric nodes/weights) + QD wire/UI ◻ | ★★ | Moderate | zero-dim root-find, exact null-space/Vandermonde |
