...
> **Status:** **ALL PHASES COMPLETE (0–6) — publish-ready.** Phase 5 (the 3D engine): analytic **landscape**
> (5A), **shading** (5B), the **∞-inspector** (F8), the **Riemann sphere** (5C / F7), and **linked 2D↔3D nav**
> (5D / I7). Phase 6 (export, interop, a11y, publish): **hi-res PNG export + metadata + copy-image (6A)**,
> **suite interop (6B, → Complex Dynamics)**, **keyboard / touch / CVD accessibility (6C)**, and the
> **publish gate (6D)** — the **3D-slice extraction ADR** ([ADR-0012](../DECISIONS.md#adr-0012)) written,
> the launcher card flipped to a live link, and the `deploy-pages.yml` `cp` line added; **the plotter goes
> live on the next merge to `master`.** See the Build-progress record below. Scope approved 2026-08: **Core +
> v1 (67 items)** as the build target; **Later + Exploratory** tracked as a backlog.
...
| **5D — linked 2D↔3D nav (I7)**          | ✅ done     | `0529310`                                  | a fourth **View** = **linked**: the flat portrait and the landscape **side by side** in one canvas (scissored split viewports, `paintLinked`), both reading the same `view` — so pan/zoom/keyboard in the flat pane moves the surface's domain in lock-step (the shared view IS the sync), while a drag on the surface pane orbits it alone. `paint2D`/`paintSurface` take a viewport rect; the coordinate helpers take an optional CSS rect (`twoDRect` = the flat pane) so a pan in the left half maps right; `main.ts` routes pointer/wheel/keyboard by half (`effMode`, pure `leftHalf`/`isLeftHalf`). Headless GT: compose (portrait \| surface-on-bg), keyboard pan changes **both** panes, a right-pane orbit changes only the surface (left diff 0.000) |
| **5 — 3D engine**                       | ✅ **done** | _all F1–F8 + I7 landed_                    | F1–F8, I7 · ~~5A-i kit~~ ✅ · ~~5A-ii landscape (F1/F5/F6)~~ ✅ · ~~5B shading (F2/F3/F4)~~ ✅ · ~~F8 ∞-inspector~~ ✅ · ~~5C-ii Riemann sphere (F7)~~ ✅ · ~~5D linked nav (I7)~~ ✅ · GT: Γ landscape + `ComplexPlot3D`-style views. _The 3D-slice **extraction ADR** (§4) is the one remaining Phase-5 artifact — three consumers now (QD/CD/plotter) — written at the 6D gate._                                                                                                                                                                                                                                                                                                                                                                             |
...
- **✅ 5D, linked 2D↔3D navigation, is landed (Phase 5 complete; I7).** A fourth **View** = `linked` renders
  the flat portrait and the landscape **side by side** in one canvas — `Plot.paintLinked` scissors the
  drawing buffer into a left (2D) and right (surface) half and paints each from the **same `view`**, so
  there is no second state to reconcile: navigating either pane through the shared `view` moves the other in
  lock-step. `paint2D`/`paintSurface` gained a viewport-rect parameter (default = whole canvas); the
  coordinate helpers (`screenToWorld`/`setCenterAtScreen`/`zoomAt`) take an optional CSS `ViewportRect`, so a
  drag/zoom in the linked flat pane maps against its half-rect. `main.ts` routes interaction by half:
  `effMode(clientX)` → `2d` (left, pan/zoom the domain — both panes follow) or `3d` (right, orbit/dolly the
  camera — the portrait is untouched), with pure `leftHalf`/`isLeftHalf` in `ui/navigation.ts`; keyboard
  drives the shared domain. Headless GT: the split composes (left portrait fills, right surface floats on
  bg), a keyboard pan changes **both** panes, and a right-pane orbit changes only the surface (left diff
  **0.000**). Persisting the 3D/linked view (mode/camera/height/rotation) in the share-link is a small
  deferred polish (those views are transient, like the sweep).
- **⏳ The one remaining Phase-5 artifact: the 3D-slice extraction ADR (§4).** QD, CD, and the plotter now
  each carry a mat4 / camera / sphere kit (three consumers), so the ADR-0007 trigger is met — but the
  extraction is a downward refactor (likely into `@cas/gpu`, or `@cas/core` whose index reserves
  "mat4/camera"; §1.6). **Write it at the 6D gate** so §7's Definition of Done ("three ADRs written") isn't
  silently marked complete; the actual code extraction can be recommended as follow-up rather than done in
  the same step.
...
- **✅ 6D, the publish gate, is landed (Phase 6 complete — publish-ready).** (1) The **3D-slice extraction
  ADR** is written ([ADR-0012](../DECISIONS.md#adr-0012)): extract the `mat4` + quaternion / arcball core to
  `@cas/gpu`, keep the app-specific 3D (mesh / height / camera / shaders) local, and migrate incrementally —
  the physical move **deferred** so no big-bang refactor lands mid-phase; §7's Definition of Done ("three
  ADRs written": [0010](../DECISIONS.md#adr-0010-complex-function-plotting-tool-as-a-separate-app) /
  [0011](#adr-0011-casexpr-named-parameters) / [0012](../DECISIONS.md#adr-0012)) now holds. (2) The launcher
  card is flipped from "Coming soon" to a `complex-function-plotter/` link (honest description — the unshipped
  residues / winding numbers are dropped), and the `cp -r apps/complex-function-plotter/dist
_site/complex-function-plotter` line is added to `deploy-pages.yml`; the plotter ships **no workers**, so
  `check-built-artifacts.mjs` is unchanged (verified). CLAUDE.md decision 11's published list is synced. **It
  goes live on the next merge to `master`** — the deploy fires on push to `master`, so this branch is
  publish-ready but nothing is live until merged. A follow-up could persist the 3D / linked / sphere view in
  the share-link (currently transient) and do the deferred ADR-0012 code extraction.
