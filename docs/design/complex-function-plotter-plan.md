...
> **Status:** IN PROGRESS — **Phases 0–4 complete**; **Phase 5 (the 3D engine) complete** — analytic
> **landscape** (5A), **shading** (5B: analytic `f'/f` normals + specular), the **∞-inspector** (F8), the
> **Riemann sphere** (5C / F7), and **linked 2D↔3D nav** (5D / I7). **Phase 6 (export, interop, a11y →
> publish) underway** — **hi-res PNG export + reproducibility metadata + copy-image (6A)**, **suite interop
> (6B: import/export a map, → Complex Dynamics)**, and **keyboard / touch / CVD accessibility (6C)** have
> landed; the **publish flip (6D)** remains — with the **3D-slice extraction ADR** (three-consumer trigger
> now met) to write alongside it. See the Build-progress record below. Scope approved 2026-08: **Core + v1
> (67 items)** as the build target; **Later + Exploratory** tracked as a backlog.
...
- **Next — 6D, publish (the phase gate).** Write the **3D-slice extraction ADR** (§4; the three-consumer
  trigger is met — recommend deferring the actual downward refactor as follow-up, so the ADR records the
  decision without a risky same-step extraction), so §7's Definition of Done ("three ADRs written") holds.
  Then flip the launcher card from "Coming soon" to a `complex-function-plotter/` link (mirror the CD/QD
  cards) and add one `cp -r apps/complex-function-plotter/dist _site/complex-function-plotter` line to
  `deploy-pages.yml`'s assemble step (the plotter ships **no workers**, so `check-built-artifacts.mjs` needs
  no change — verified). Publishing is outward-facing — pause for an explicit go-ahead on the launcher flip
  itself.
