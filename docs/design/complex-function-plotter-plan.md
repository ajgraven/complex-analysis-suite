...
> copy-image (6A)** and **suite interop (6B: import/export a map, → Complex Dynamics)** have landed; a11y
> (6C) precedes the publish flip (6D). See the Build-progress record below. Scope approved 2026-08: **Core +
...
- **Next — 6C, accessibility (L7/L8).** Keyboard navigation (pan/zoom/reset via arrows / +− / Home) and
  touch gestures (pinch-zoom, drag-pan; touch orbit/arcball in 3D/sphere), plus surfacing the CVD-safe
  colormap as a discoverable default (L8; the CVD-sim preview stays). Then 6D (publish) — where the parked
  **5D linked-nav (I7) + the 3D-slice extraction ADR** get re-surfaced before the Definition of Done (§7).
