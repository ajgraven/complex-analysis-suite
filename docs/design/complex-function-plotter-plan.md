...
> **landscape** (5A: orbitable height surface, top-down = the 2D portrait) has landed; analytic normals +
> shading (5B), the Riemann sphere (5C), and linked nav (5D) are next. See the Build-progress record below.
...
| **5A-ii — analytic landscape**          | ✅ done            | _⚠ backfill hash next commit_                    | the surface render (F1/F5/F6): grid mesh vertex-displaced by height (log \| linear \| stereographic), fragment recomputes `f` + reuses `colorAt`; `Plot` gains a 2d/3d `mode` + orbit/dolly + height UI; simple dFdx/dFdy shading (off top-down). Headless GT: **top-down == 2D pixel-for-pixel (maxDiff 0)**, orbit works |
| **5 — 3D engine**                       | 🔨 **in progress** | _5A landscape done; 5B shading / 5C sphere next_ | F1–F8, I7 (+ the 3D-slice extraction ADR) · ~~5A-i kit~~ ✅ · ~~5A-ii landscape (F1/F5/F6)~~ ✅ · next: 5B analytic normals `f'/f` + shading (F2/F3/F4) · 5C Riemann sphere (F7/F8) · 5D linked nav (I7) + extraction ADR                                                                                                  |
...
**Resume notes — Phase 5 underway (3D engine): 5A (the analytic landscape) landed; 5B shading / 5C sphere next:**
...
- **✅ 5A-ii, the analytic-landscape render, is landed (5A complete).** `render3d/height.ts` +
  ...
  image, 2D restores. _First thing next commit: backfill 5A-ii's hash into the build-progress row above._
- **Next — 5B, on-surface shading.** The analytic normal from `f'/f` via `@cas/expr` `differentiate`
  (F4 — replacing the geometric dFdx/dFdy normal), hue-preserving multiply-shading + optional specular
  (F2), and the on-surface enhanced overlay (F3). Then 5C (Riemann sphere, F7/F8) and 5D (linked 2D↔3D
  navigation I7 + the 3D-slice extraction ADR). Persisting the 3D view (mode/camera/height) in the
  share-link is a small deferred polish (the 3D view is currently transient, like the sweep).
