...
> **Status:** IN PROGRESS — **Phases 0–4 complete**; **Phase 5 (the 3D engine) underway** — the analytic
> **landscape** (5A), its **shading** (5B: analytic `f'/f` normals + specular), and the **∞-inspector**
> (F8: plot f(1/z)) have landed; the **Riemann sphere** (F7) and linked nav (5D) are next. See the
> Build-progress record below. Scope approved 2026-08: **Core + v1 (67 items)** as the build target;
> **Later + Exploratory** tracked as a backlog.
...
| **5C-i — ∞-inspector (F8)**             | ✅ done            | _⚠ backfill hash next commit_              | plot **f(1/z)** so the origin shows the map near ∞: a new `@cas/expr` `substitute(node, name, repl)` (capture-safe) does `z → 1/z` on the AST, so the 2D + surface GPU and the CPU instruments all track the same map; an `Inspect ∞` checkbox. Headless GT: on `f(z)=z` the origin flips from a dark zero to a bright pole, the portrait inverts (98.7%), restores                                        |
| **5 — 3D engine**                       | 🔨 **in progress** | _5A + 5B + F8 done; 5C-ii sphere next_     | F1–F8, I7 (+ the 3D-slice extraction ADR) · ~~5A-i kit~~ ✅ · ~~5A-ii landscape (F1/F5/F6)~~ ✅ · ~~5B shading (F2/F3/F4)~~ ✅ · ~~F8 ∞-inspector~~ ✅ · next: 5C-ii Riemann sphere (F7) · 5D linked nav (I7) + extraction ADR                                                                                                                                                                             |
...
**Resume notes — Phase 5 underway (3D engine): 5A landscape + 5B shading + F8 ∞-inspector landed; the Riemann sphere (F7) next:**
...
- **✅ F8, the ∞-inspector, is landed (5C-i).** A new `@cas/expr` `substitute(node, name, replacement)`
  ...
  pixels), no compile error, and it restores. _First thing next commit: backfill F8's hash above._
- **Next — 5C-ii, the Riemann sphere (F7).** Per-fragment ray-cast of an analytic unit sphere coloured by
  the same `colorAt` so ∞ is literal (adapting CD's `sphereView` quaternion arcball + `@cas/core`'s
  `planeToSphere`/`sphereToPlane`), added as a third view mode (2D / landscape / sphere). Then 5D (linked
  2D↔3D navigation I7 + the 3D-slice **extraction ADR** — QD, CD, and the plotter now each carry a mat4 /
  sphere kit, the three-consumer trigger). Persisting the 3D view (mode/camera/height) in the share-link
  is a small deferred polish (the 3D view is currently transient, like the sweep).
