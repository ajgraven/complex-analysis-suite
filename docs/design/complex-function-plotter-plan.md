...
> **landscape** (5A), its **shading** (5B: analytic `f'/f` normals + specular), the **∞-inspector**
> (F8: plot f(1/z)), and the **Riemann sphere** (5C / F7) have landed; linked 2D↔3D nav (5D) is next. See
> the Build-progress record below. Scope approved 2026-08: **Core + v1 (67 items)** as the build target;
...
| **5C-ii — Riemann sphere (F7)**         | ✅ done            | _⚠ backfill hash next commit_              | the extended plane ℂ∪{∞} as a literal **sphere**: a fullscreen per-fragment **ray-cast** of the unit sphere, stereographically projected (south=0, equator=\|z\|=1, **north=∞**) and coloured by the same `colorAt`; a quaternion **arcball** drag (ported from CD `sphereView`) + dolly, added as a third `mode` behind a 3-way **View** toggle. Control flow stays uniform (masked miss, normalized hit) so `colorAt`'s `fwidth` is defined. Headless GT: lit silhouette on the app bg, z=0 centre dark, arcball rotates 51.7%, dollies, 2D restores |
...
- **Next — 5D, linked 2D↔3D navigation (I7) + the 3D-slice extraction ADR.** A synced side-by-side (pan/zoom
  in the flat view drives the surface's domain and back). QD, CD, and the plotter now each carry a mat4 /
  sphere kit — the **three-consumer trigger** for the extraction ADR (likely into `@cas/gpu`, or `@cas/core`
  whose index already reserves "mat4/camera"; §1.6 / §4). Persisting the 3D view (mode/camera/height/sphere
  rotation) in the share-link is a small deferred polish (the 3D + sphere views are currently transient, like
  the sweep).
