...
> **Status:** IN PROGRESS — **Phases 0–4 complete**; **Phase 5 (the 3D engine) underway** — its pure
> math kit (`render3d/`: mat4 · orbit camera · grid mesh) has landed; the analytic-landscape renderer is
> next. See the Build-progress record below. Scope approved 2026-08: **Core + v1 (67 items)** as the build
> target; **Later + Exploratory** tracked as a backlog.
...
| **5A-i — 3D math kit**                  | ✅ done            | _⚠ backfill hash next commit_              | app-local `render3d/` foundation: typed column-major `mat4` (+ `ortho`/`transformPoint`, ported from QD `sphere-common`), the F5 orbit camera (Z-up; exact top-down ortho = 2D portrait, unit-proven), and `buildGridMesh`; pure + fully unit-tested (no renderer yet — the 2D app is untouched)        |
| **5 — 3D engine**                       | 🔨 **in progress** | _5A-i (kit) done; landscape renderer next_ | F1–F8, I7 (+ the 3D-slice extraction ADR) · ~~5A-i 3D math kit~~ ✅ · next: 5A-ii the analytic-landscape surface render + 2D/3D toggle + orbit/dolly (F1/F5/F6)                                                                                                                                         |
...
**Resume notes — Phase 5 underway (3D engine): 5A-i (3D math kit) landed; the landscape renderer is next:**
...
- **Phase 5 underway — the 3D engine (sub-milestones 5A landscape → 5B shading → 5C sphere → 5D linked
  ...
  (`buildGridMesh(n)`). Pure + fully unit-tested; the 2D app is untouched. _First thing next commit:
  backfill 5A-i's hash into the build-progress row above._
- **Next — 5A-ii, the analytic-landscape surface render.** A new WebGL2 surface program consuming the kit:
  the grid mesh, a vertex shader that evaluates `f` at each grid point and displaces by a height
  compression (F1: log|f| clamped · linear · bounded stereographic `(|f|²−1)/(|f|²+1)`), a fragment shader
  that **recomputes `f` from the interpolated world (re, im) and reuses `colorAt`** (so the surface colour
  is pixel-crisp and top-down matches 2D pixel-for-pixel — the gate), plus a **2D/3D view toggle** and
  drag-orbit / wheel-dolly. Simple geometric (dFdx/dFdy) shading first; **F4 analytic normals from `f'/f`**
  and F2/F3 shading + on-surface overlays are 5B. Reuse: `colorAt` + the `COMPLEX_*`/`fFn` GLSL already
  compile in a vertex stage too (no `fwidth` there — that stays in the fragment).
