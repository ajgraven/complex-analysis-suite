...
> **landscape** (5A: orbitable height surface, top-down = the 2D portrait) and its **shading** (5B:
> analytic `f'/f` normals + specular) have landed; the Riemann sphere (5C) and linked nav (5D) are next.
> See the Build-progress record below. Scope approved 2026-08: **Core + v1 (67 items)** as the build
...
| **5B — on-surface shading**             | ✅ done            | _⚠ backfill hash next commit_                  | **analytic normal** from `f'/f` (F4, `differentiate`→`fpFn`; geometric fallback for Γ/ζ/anti-holo), wrap-diffuse + optional **specular** (F2), on-surface enhancement (F3, free via `colorAt`). Fixed a link error: `uHeightMode` (int, both stages) needed `precision highp int` — a unit-test guard now pins it. Headless GT: z² analytic + Γ geometric render, specular brightens, top-down still == 2D |
| **5 — 3D engine**                       | 🔨 **in progress** | _5A + 5B done; 5C sphere / 5D linked-nav next_ | F1–F8, I7 (+ the 3D-slice extraction ADR) · ~~5A-i kit~~ ✅ · ~~5A-ii landscape (F1/F5/F6)~~ ✅ · ~~5B shading (F2/F3/F4)~~ ✅ · next: 5C Riemann sphere (F7/F8) · 5D linked nav (I7) + extraction ADR                                                                                                                                                                                                     |
...
**Resume notes — Phase 5 underway (3D engine): 5A landscape + 5B shading landed; 5C Riemann sphere next:**
...
- **✅ 5B, on-surface shading, is landed (F2/F3/F4).** `Plot.compileSource` now also compiles
  ...
  **analytic normal** `n = normalize(−H'(|f|)·∇|f|, 1)` with `∇|f| = (Re,−Im)(conj(f)·f')/|f|` (F4, smooth
  ...
  brightens ~36% of pixels, top-down maxDiff 0. _First thing next commit: backfill 5B's hash above._
- **Next — 5C, the Riemann sphere.** Per-fragment ray-cast of an analytic unit sphere coloured by the same
  `colorAt` so ∞ is literal (F7, adapting CD's `sphereView` quaternion arcball + `@cas/core` `planeToSphere`),
  and the cheap `f(1/z)` ∞-inspector first (F8). Then 5D (linked 2D↔3D navigation I7 + the 3D-slice
  extraction ADR). Persisting the 3D view (mode/camera/height) in the share-link is a small deferred polish
  (the 3D view is currently transient, like the sweep).
