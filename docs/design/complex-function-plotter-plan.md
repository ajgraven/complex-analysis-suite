...
>
> - copy-image (6A)** has landed; interop (6B) + a11y (6C) precede the publish flip (6D). See the
...
- **Next — 6B, interop (K7/K8).** Accept an `@cas/interchange` **map Envelope** (`#s=` / JSON) — a Schwarz σ
  or φ from QD, or a generic `expr`/`rational`/`laurent` map — by porting CD's `envelopeToMapSpec` +
  `mapSpecToExpr` consumer path into the plotter (K7); and **emit** a View + a "send to Complex Dynamics"
  deep-link (K8; note **CD has no interchange emit** to copy — QD's `schwarz-export.mjs` is the emit
  reference, and payloads travel in the CANONICAL convention, ADR-0006). GT: a σ imported from QD renders; a
  View exported round-trips. Then 6C (a11y: L7 keyboard/touch · L8 CVD-safe defaults) and 6D (publish).
