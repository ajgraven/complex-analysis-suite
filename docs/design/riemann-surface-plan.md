...
| **M1 — parametrize-by-w Riemann-surface mode** | ✅ done | new `Plot.mode "riemann"` + `paintRiemann` + orbit-camera auto-framing; height-source (Re w / Im w, auto-defaulted per form); sheet-count control (infinite families, badged); exaggeration; coloring parity (shared `colorAt` — colormaps/enhancements carry over); auto-detect enables the tab + honest form/monodromy/cut badge, falls back to principal-branch views otherwise; orbit/dolly pointer + keyboard; permalink extension (`riemann` mode + `riemannHeight`/`riemannSheets`, back-compatible). Verified in the live app (screenshots): √z interlock, log helicoid, z^(1/3) three sheets. **Deferred within M1 (flagged at gate): the 3D hover-pick readout on the multi-sheet surface and the side-by-side linked base-plane pane — both fold into M3.** |
| **M1 gate** | ✅ green · pushed | full repo gate green — lint (+dep:check, 907 modules, 0 violations) · **385 test files / 3220 tests** · build (all apps, built-artifact gate OK); the top-down-3D≡2D golden + all existing tests unchanged. Pushed to `claude/riemann-surface-rendering-fvybo6`. |
| **M1 review + fixes** | ✅ done | Self-review + independent adversarial review. Root-cause fix for the `z^(p/q)` family: **charisma height now lifts by the uniformizer `t`, not the value `w = t^p`** (was a `\|t\|^p` needle for p≥2), which also bounds the camera framing; `z^(p/q)` **window is now `q`-aware** (corner `\|z\|=\|t\|^q` capped, no giant lobes / float overflow for high roots); framing distance clamped; charisma/sheet choices preserved across same-primitive edits; stale cursor readout cleared in Riemann/sphere; monodromy label uses `\|p\|`. Docs brought current (README five-way toggle + Riemann paragraph; plotter plan F12 done; `main.ts` header). Verified: `z^(3/2)`/`z^(2/3)` render as proper bounded multi-sheet surfaces. **Paused for review before M2.** |
