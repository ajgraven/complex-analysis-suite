...
| **M1 — superposition sandbox** | ◐ in progress (S1–S4 done) | **Done:** pan/zoom + draggable singularities with live recompute (S1); palette (Source/Sink/Vortex/Doublet) + uniform stream + inspector editing the monopole as `c = q+iγ` with the live residue decomposition (S2); the Electrostatic ↔ Fluid two-lens toggle + persistent legend (S3); the flux/circulation **probe** (drag a loop → exact `∮ E dz = Σ residues` = Gauss's law (Re) + Kelvin circulation (Im)) + its theorem caption — the first theorem-gallery entry (S4). **Remaining:** presets (Rankine, cylinder+circulation), `#vs=` permalink + PNG export; optional sensor puck, even-spaced streamlines / motion layer. All verified in live headless-Chromium WebGL2; ~30 node tests. |
