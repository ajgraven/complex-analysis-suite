...
| **M1 — superposition sandbox** | ✅ done (S1–S5) | pan/zoom + draggable singularities with live recompute (S1); palette (Source/Sink/Vortex/Doublet) + uniform stream + inspector editing the monopole as `c = q+iγ` with the live residue decomposition (S2); the Electrostatic ↔ Fluid two-lens toggle + persistent legend (S3); the flux/circulation **probe** (drag a loop → exact `∮ E dz = Σ residues` = Gauss's law (Re) + Kelvin circulation (Im)) + its theorem caption, the first theorem-gallery entry (S4); presets (Rankine half-body/oval, cylinder ± circulation, empty), `#vs=` permalink + copy-link + PNG export with embedded recipe (S5). All verified in live headless-Chromium WebGL2; 33 node tests. **Deferred-optional within M1:** a sensor puck and an even-spaced-streamline / particle-motion layer (the φ/ψ contour net serves as the streamline view for now). |
| **M1 gate** | ◐ functionality complete — awaiting review | Full repo gate green (lint + dep:check 968 modules, typecheck, **412 test files / census `2d-electrostatics:8`**, build all apps). **Publish-wiring held for the review decision** (outward-facing): registering in `SUITE_APPS` (+ its nav test), the launcher card, the `deploy-pages.yml` `cp`, an in-app `mountNavHeader`, and the CLAUDE.md/README status bump — all done on approval so the app ships on the next `master` merge. **Pause for review.** |
