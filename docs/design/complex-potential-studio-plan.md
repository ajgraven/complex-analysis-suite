...
| M2 — conformal transplant | ◐ in progress | **M2.1 (done):** the closed-form Joukowski airfoil transplant engine (`src/airfoil.ts`) — cylinder-plane potential/velocity, Joukowski map + exterior-branch inverse, the Kutta circulation `Γ = 4πUR·sin(φ₀−α)`, and lift `L = ρUΓ`, node-tested against ground truth. **Next:** the two-pane cylinder↔wing render (push-forward), Kármán–Trefftz, then interior/exterior SC + the `ConformalMap` interchange form. |
