...
| M2 — conformal transplant | ◐ in progress (M2.1–M2.3 done) | **M2.1:** the closed-form Joukowski airfoil engine (`src/airfoil.ts`) — cylinder potential/velocity, Joukowski map + exterior-branch inverse, Kutta circulation `Γ = 4πUR·sin(φ₀−α)`, lift `L = ρUΓ`. **M2.2:** the two-pane **cylinder↔wing** render (`airfoil.html` + `main-airfoil.ts`, Vite multi-page) — one fragment shader with a `uMode` switch renders both planes, contouring the SAME stream function so streamlines map across; thickness/camber/AoA + Kutta → live lift. **M2.3:** **Kármán–Trefftz** finite trailing-edge angle (`n = 2 − τ/π`; a TE-angle slider), with ζ and K' built from the same `m` so the non-integer power leaves no seam. 12 airfoil tests; all verified live (`L = −2.705`, finite TE wedge). **Next (M2.4):** interior/exterior SC transplant (flow in/past a polygon via `@cas/conformal`, flat plate = degenerate) + the `ConformalMap` interchange form (**ADR-0034**) — the architectural piece that opens the deferred cross-app hand-off. |
