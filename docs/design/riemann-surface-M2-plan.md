...
|---|---|---|
| **M2.0 — algebraic-curve spike** | ⏳ pending | `sqrt(z^2−1)` end-to-end: recognize `R(z)^(1/n)`; per-vertex sheets = n-th roots of `R(z)`; triangulate → proximity-stitch → local near-degenerate subdivision + ramification holes; **Web Worker** mesh-gen → baked-mesh upload → curve render. No new package deps (branch points via the local degeneracy test). |
