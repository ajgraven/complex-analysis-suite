...
| **C2 — permutation diagram** | ✅ done | `src/riemann/permDiagram.ts` (canvas: sheet-coloured nodes `1…n` + bowed `k → σᵢ(k)` arrows, matching the surface's sheet-hue law; `role="img"` + cycle-notation aria-label; 5 tests). The group summary now shows a diagram per generator (γᵢ, γ∞). Bonus: parametric branch points now come from the **exact** cut-ray origins (`Plot.riemannParamBranchPoints`) instead of the mesh-limited scan — `z^(1/3)` went from 24 spurious branch points to the correct 1 (a real 3-cycle diagram, C₃, genus 0). |
...
| **C4 — monodromy report panel** | ✅ done | a theme-aware full-screen **Monodromy report** (opened from the inline summary): the covering's fingerprint (sheets · branch points · genus · group · connected), the π₁ generators gallery (diagram + cycle + branch-point location per γᵢ, plus γ∞), and the **Riemann–Hurwitz computation worked out with the actual numbers**. Refactored a shared `gatherMonodromy()` behind both the inline summary and the report. Layout only, no new math. |
