...
| **M3.4 — legibility polish (branch-point markers)** | ✅ done | **Branch-point markers** (moved here from M3.2): `src/riemann/branchPoints.ts` locates ramification uniformly for both paths — a **sheet-separation scan** (the small-separation local minima where sheets merge, incl. a fully-merged interior node) via the shared `sheetsAt` enumerator, clustered + locally refined, `≈`. `Plot.riemannBranchPoints()` scans the surface's base extent (curve z-view / parametric xy-bounds); `main.ts` draws them as amber ⊕ on the base-plane pane and appends the count to the badge (`≈K branch points`). **Per-sheet tint and cut-shadow were considered and declined** on honesty grounds: a *global* sheet identity is exactly what monodromy permutes (M3.3), and a branch *cut* is a choice not an invariant — a per-sheet tint would seam and a drawn cut would over-claim; the invariant, honest thing to show is the branch *points*. Node: 5 tests (√(z²−1)→±1, √z→0, no-merge→none, coverage-hole→no edge artifacts, degenerate box). |
| **M3.4 gate** | ✅ green · pushed | full repo gate green — typecheck · lint (+dep:check, **no new edges**) · **389 files / 3259 tests** · build (all apps); plotter browser goldens (38) pass; existing tests unchanged. **All of M3 (M3.1–M3.4) complete.** |
