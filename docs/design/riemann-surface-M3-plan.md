...
|---|---|---|
| **M3.1 — multi-sheet hover-pick value inspector** | ⏳ in progress | ray-cast the drawn Riemann surface (both the M1 parametric and the M2 baked-curve paths) under the cursor; report the point actually on the surface — `z`, `w`, `\|w\|`, `arg w`, and a **local branch ordinal** (which of the N sheets over this `z` you're on). Pure-geometry pick module (Möller–Trumbore + point-in-triangle sheet census); `≈`-labeled. |
