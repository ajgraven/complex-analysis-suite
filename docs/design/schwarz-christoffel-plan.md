...
- ~~**Riemann-map app wiring:** replace the smooth-only region source with the SC engine so a polygon
  ...
  **Deepened since (Phases A–C):** reentrant presets (L-shape, cross); the **exact inverse** wired into the
  Ω→𝔻 direction **for polygon domains** (both directions now exact for polygons, not lightning-approximate —
  smooth regions keep the lightning fit by design); the **prevertex ↔ corner** correspondence overlay with
  interior-angle labels + hover-linking; and a **draggable-vertex custom-polygon editor** in the primary panes
  (fast-drag → precise-on-release warm-started continuation, serialized in `#vs=`).
