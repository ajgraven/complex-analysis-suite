...
> **interactive field-and-flow** app: the free-field sandbox + the conformal-transplant airfoil and
> polygon. The full M0–M4 construction record — including the parts that moved — lives in
...
map — past a Joukowski / Kármán–Trefftz airfoil, or past/inside a polygon via Schwarz–Christoffel. Three
pages, all **steady** (a fixed field or a fixed map); the *evolving* free-boundary story is Hele-Shaw
Flow's, and the *analysis* of a conductor is Potential Theory's.
...
| `airfoil.html` | Flow past a Joukowski / Kármán–Trefftz airfoil (the cylinder↔wing transplant; Kutta lift). |
...
- **M2 — conformal transplant.** The closed-form Joukowski/Kármán–Trefftz airfoil engine; the exterior +
  interior Schwarz–Christoffel polygon transplant (now via `@cas/flow`); the `ConformalMap` interchange
  form (ADR-0035) — the app is producer (Copy link) and consumer (import a `#s=` polygon) and the
  Riemann-map SC studio is the cross-app producer (`RM_TO_POTENTIAL_CONFORMAL_LINK` golden).
...
- **ES-4 — more transplant families.** Additional closed-form maps (slit, ellipse, star) as first-class
  transplant targets alongside the airfoil and polygon.
