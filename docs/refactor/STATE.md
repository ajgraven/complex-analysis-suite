...
- **installAlgebra decomposition UNDERWAY** (QD-ALG-1; user chose "Carve-outs" + "keep carving") — attack the
  ...
  **Carve-out 3 (`exactValueStr` + `fmtRat` → NEW `algebra-format.mjs`) — MERGED (#197).** **Carve-out 4 (`fmtRatio` +
  ...
  + `_parseMomentNum` moment parser → NEW `algebra-moment-parse.mjs`) — PR #199 OPEN.**
...
- Cadence: merge on green (user delegates). `APPROVED: PLAN.md v1`.
  Roadmap §8: A✓ / B✓ / C✓ / **D (ui.mjs seams✓; installAlgebra decomposition underway — carve-outs 1✓ 2✓ 3✓ 4✓ 5(open))** / E / F.

## Branches / PR
- Integration `refactor/main` @ **821d39d** (this STATE commit advances it). Tree clean.
- **OPEN PR #199** — `refactor/d-alg-carve-5-moment-parse` @ 58de4dd (off 821d39d): carve-out 5. CI running
  (browser in_progress / build queued at open); subscribed + 12-min merge-on-green fallback armed (19:55Z).
- Merged stage PRs (21): A1 #178 … D-alg-carve-3 #197, **D-alg-carve-4 #198 (50216af)**.

## Validation state (green bar)
- **`refactor/main` @ 821d39d — ALL GREEN:** build/typecheck/lint exit 0; `pnpm test` **2174 passed / 248 files**.
- **PR #199 stage branch @ 58de4dd — ALL GREEN (firsthand):** build/typecheck/lint exit 0; `pnpm test`
  **2183 passed / 249 files** (+9, +1 file — new headless `algebra-moment-parse.test.ts`). Lands on merge.

## Uncommitted / unverified
- None. Carve-out 5 committed to its stage branch (58de4dd) + pushed; this STATE commit direct to `refactor/main`.

## Known blockers / risks
- PR #199 open, CI in progress; drive-to-green posture (my PR). No blockers.
...
## Next concrete steps
1. **Merge #199 on green** (fallback armed 19:55Z). On merge → advance `refactor/main`, re-confirm green, refresh STATE.
2. **Carve-out 6** (census next): `withGuidance` + `_isCapFailure` — the honest-labeling failure-guidance predicate +
   decorator (~19 call sites; could join `algebra-labeling.mjs`). Then `_pronyLatex`, `valStr`+`substList`, cheap stragglers.
3. Group order: A✓ B✓ C✓ → **D (installAlgebra decomposition underway — 5 carve-outs; census: ~15 pure left)** → E → F.
