...
| ID | Sev | Class | Finding | Evidence |
|---|---|---|---|---|
| QD-UI-1 | high | design-problem | Worker lifecycle duplicated 6× (3 lanes in primary-solver-worker + schwarz/sym/param-slice); drifted (primary has `messageerror`, aux/live don't) and **already shipped the "stuck on Pass 1/3" Schwarz bug** fixed in only one copy · **[B4-2a+b: crash contract pinned for all 3 solver lanes (psw-crash-char + sym-worker-crash-char); messageerror asymmetry + sym F4 frozen; schwarz/pool P2 → B4-2c/Group C; the 6× dedup itself → Group C]** | primary-solver-worker.mjs:80/208/302,113; schwarz-cpu-worker.mjs:83-96 |
