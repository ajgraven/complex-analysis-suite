...
| ID | Sev | Class | Finding | Evidence |
|---|---|---|---|---|
| QD-SOLV-1 | high | design-problem | Dispatch precedence = reverse load order (`registerFamily` unshift); seed→solver order **triplicated** across 3 hand-maintained lists; a `_singular` mis-ordered before its base → silently wrong φ. Nothing asserts base-before-singular · **[GUARDED · stage A2 — assertDispatchOrder; list triplication → E2]** | solver.mjs:1283-1285; main.mjs / workers/solver-graph.mjs / test/bootstrap.js |
...
| ID | Sev | Class | Finding | Evidence |
|---|---|---|---|---|
| QD-UI-1 | high | design-problem | Worker lifecycle duplicated 6× (3 lanes in primary-solver-worker + schwarz/sym/param-slice); drifted (primary has `messageerror`, aux/live don't) and **already shipped the "stuck on Pass 1/3" Schwarz bug** fixed in only one copy · **[B4-2a+b: crash contract pinned for all 3 solver lanes (psw-crash-char + sym-worker-crash-char); messageerror asymmetry + sym F4 frozen; schwarz/pool P2 → B4-2c/Group C; **PSW 3× dedup DONE — C1a `createWorkerLane`, 395→238; sym/schwarz/pool dedup → C1b**]** | primary-solver-worker.mjs:80/208/302,113; schwarz-cpu-worker.mjs:83-96 |
...
| ID | Sev | Class | Finding | Evidence |
|---|---|---|---|---|
| QD-TEST-1 | high | design-problem | Node suite = one serial child process (`execFileSync`) → 2302 asserts in 1 Vitest test on 1 core; no parallelism/reporting/isolation; watch reruns full ~128s · **[B1: split into 26 parallel specs — reporting/isolation ✓; B2: solvers sharded → `pnpm test` 157s→109s (−30%), long-pole 77s→37s ✓]** | node-suite.test.ts:14; node-test.js:97-104 |
...
| QD-TEST-4 | med | design-problem | `ui-solve.mjs` orchestration (`installSolve` DOM+worker closure) untested — only the pure badge helper is · **[B4-1 ✓ (d17e9df): 12-test orchestration net — guards/dispatch/settle/supersede/busy/cancel/escalate; mutation-verified; no source change]** | ui-solve.mjs:49,51 |
| QD-TEST-5 | med | optional | `solvers.test.js` ≈76s is the floor unless the file itself is sharded (independent blocks verified) → shard → ~25–30s · **[B2 ✓ (ac5f894): 4 contiguous shards; long-pole 37s — §PB [826-935] is one atomic ~38s block that caps it above the ~25–30s target; suite −30%]** | solvers.test.js:677-1800 (now solvers-{1..4}.test.js) |
