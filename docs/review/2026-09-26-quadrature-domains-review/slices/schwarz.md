...
- Evidence (measured, `famexport.mjs`: QD solve → `exportSigmaLink` → `decodeLink` → CD's own `schwarzEngineFromMapSpec`):
  ...
  `vitest/schwarz-export.test.ts:152-173,265` covers weighted *bounded* φ only.
- Confidence: high
...
- Location and claim:
  ...
  - `SCHWARZ_FORMULATION.md:14-21`: uses σ for the Schwarz *function* (σ(w)=w̄ on ∂Ω), where the Schwarz tab, its README and `@cas/schwarz` use σ for the *reflection* conj(S). A reader moving between the two gets conj-flipped formulas. I re-derived the residue claim `C_{1,1}=A·φ′(z_j)=|A|²/(1−|z_j|²)²` and it is correct.
- Confidence: high
- Prior review: new
