...
- [x] **A1 CD-RENDER** — `apps/complex-dynamics/src/render/*` perf rewrite (#294) → `findings/A1-cd-render-perf.md` — 1 MED (fieldAt Fix-L field pre-pass omits the periodicity bailout colorAt has → re-iterates every interior pixel to full cap (≤20000, ×~13 df64) on first appearance-change build: perf regression + GPU-watchdog risk + latent parity edge), 3 LOW (two-pass recolour aborts temporal-AA accum; recolorParity test covers only a subset of modes; ...), 2 NIT. Rewrite correct — no wrong-image regression; σ 512→4096 cap intact.
...
- [x] **A7 CONSOLIDATION** — whole-suite duplication + perf anti-patterns + verify landed consolidations → `findings/A7-consolidation-perf.md` — 3 LOW (corr CPU escape loops never got sqrt-free treatment; CD JuliaMetricsClient send-side not coalesced like QD O1; peephole doc understates plotter reach), 2 NIT. 3 landed consolidations BIT-CLEAN; no new high-value duplication; QD branchTaylorAccumulate + sqrt-free clearance rewrites verified CORRECT ("allocation-free" is a doc overstatement — 8 typed arrays/call).
- [x] **A8 CD-INTERNALS** — CD df64 (`glPlot`/`bla`) + Julia/σ overlays + `@cas/dynamics` + state/ui (coverage gap) → `findings/A8-cd-internals.md` — 3 LOW (angles.ts compare() overflows 2^53 for large denominators → silently-wrong core entropy, reachable via spider-angle box; perturbMultibrot/perturbPoly CPU oracles hardcode escapeR>4 vs uPerturbEscape2; CPU σ lib defaults diverge from GPU), 4 NIT. df64/BLA/Zhuoran-rebasing/@cas/dynamics re-derived CORRECT; no wrong-deep-zoom-image bug; σ 512→4096 fixed; no WebGL leak regression.
