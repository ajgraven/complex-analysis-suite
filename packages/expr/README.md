...
A CindyScript-compatible subset over complex numbers, with variables `z`, `c`, and a live
free parameter `a`:
...
`test/` — parser/AST, evaluate↔compile, derivative, LaTeX, param-`a` handling, and
**`complexParity`** (the JS complex library vs. its intended GLSL semantics). The end-to-end
