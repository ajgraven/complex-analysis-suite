...
**Full suite passing: 1343 passed / 0 failed; `npm run lint` clean;
`npm run version:check` clean** (cache hash `61e7fb1c05`). `main` is at the four
most-recent roadmap merges, newest first (all on `main`, each its own merged PR):
- **UX tiers 1–2** (PR #38) — usability/clarity overhaul (status-panel dock,
  Overlays "Layers" card, example-led first run + coachmark, domain-type summary,
  intro popover, tab subtitles, solve-phase feedback). UI-only.
- **#8/#9** (PR #37) — thesis-example pack with analytic oracles + annotated-phenomena
  overlay (+ `QD.detectSymmetry`).
- **#11** (PR #36) — solver accuracy near cusps (adaptive identity escalation, Newton
  conditioning, c\* confidence, near-cusp reporting).
- **#2** (PR #35) — Tier-0 analysis primitives (`observables.js`).
...
(NOT on `main`) — don't merge it without an explicit ask. Open roadmap ideas live in
`TODO.md` and the in-chat UX menu (tiers 1–2 done; tier 3 — touch/mobile, unified
export, preference persistence, a11y, more shortcuts — still open). A **Faber-polynomials
for UQD** card + **exact-c\*** tweak is implemented on `feature/faber-polynomials` (full suite
**1364/0**, cache hash `e711528773`; see the most-recent entry) — **not yet merged**.
...
"Estimate max c" lands `state.c` exactly on c\* (1.4504357922817606 for the cardioid). On branch
**`feature/faber-polynomials`** (not yet merged).
