...
clean** (run `npm test` for the live count). Most recent work is the two
tech-debt phases below (release hardening, test-harness split); the feature work
(preimage tree, continuation, …) follows as priors.
...
## (most recent) Phase 2 — async test harness + node-test.js split
