...
export const GLOSSARY: GlossaryEntry[] = [
  ...
  },
  {
    id: "mating",
    term: "Mating",
    defn: "Glue two filled Julia sets along their boundaries (by the angle-doubling identification of external rays) to build a single rational map that combines both dynamics. Two post-critically-finite quadratics z²+c₁, z²+c₂ admit a (conformal) mating iff c₁ and c₂ are NOT in complex-conjugate limbs of the Mandelbrot set (Rees–Shishikura–Tan Lei). For main-cardioid bulbs, the p/q limb's conjugate is the (q−p)/q bulb, so the obstruction is p₁/q₁ + p₂/q₂ = 1; the 1/2 bulb is the only self-conjugate one, so every other bulb can be mated with itself. Beyond the mateability test, the app can also RENDER a mating: the mated rational map is computed by the marked-point Thurston pullback (Wolf Jung's algorithm — iterate the formal-mating pullback; the postcritical points collide but the rational maps converge), then drawn as a Julia set on the dynamical plane / Riemann sphere, where the two parent Julia sets appear on the two hemispheres. With the basilica (z²−1) as the second parent — named examples (z²+i ⊔ basilica = (z²+2)/(z²−1); rabbit ⊔ basilica = (z² − e^{2πi/3})/(z²−1)) or any satellite bulb p/q ⊔ basilica — the map has the closed form (z²−x₁)/(z²−1), and a conjugation-symmetry gate (x₁(c̄)=conj(x₁(c))) refuses a bulb whose result isn't self-consistent. For a GENERAL second parent — mate two arbitrary hyperbolic bulbs p₁/q₁ ⊔ p₂/q₂ — both critical values are free, so the map is the Boyd–Henriksen form F(z) = (v·z² − u·k)/(z² − k), k=(v−1)/(u−1), with u,v solved by the same pullback tracking BOTH parents' postcritical orbits. Obstructed pairs are refused (Tan Lei), and the correct map is picked by swap-consistency: the mating is order-independent up to z↦1/z (which sends (u,v)↦(1/v,1/u)), which for a diagonal mating A⊔A is exactly u·v=1. Hyperbolic parents are supported; a Misiurewicz second parent and the full slow-mating homotopy are still in progress.",
  },
  {
    id: "misiurewicz",
  ...
  {
    id: "lamination",
    term: "Lamination (pinched-disk model)",
    defn: "Thurston's combinatorial model of a quadratic Julia set. The Böttcher map parametrises the exterior by an external angle θ ∈ ℝ/ℤ, on which f acts by doubling. When two external rays land at the same point of ∂K_c their angles are identified; joining every such pair by a chord (a leaf) in the unit disk gives the lamination — a pinched disk whose quotient (collapse each leaf) is homeomorphic to the Julia set. A point where q rays co-land is an ideal q-gon gap; the app draws all its sides. The set of leaves is invariant under doubling and under z ↦ −z (θ ↦ θ+½). Shown as a corner disk widget on the dynamical plane, alongside the Julia set it models. The app measures the leaves honestly — it lands the (pre)periodic rays with the same Newton tracer used elsewhere and joins the ones that co-land — so the picture is a faithful finite-detail approximation (higher-period pinches beyond the detail bound are simply not drawn). z²+c, and only when α is repelling (c outside the main cardioid). Basilica (c = −1): the α leaf {1/3, 2/3} and the −α leaf {1/6, 5/6}; the rabbit's α is an ideal triangle {1/7, 2/7, 4/7}.",
  },
  {
    id: "qml",
    term: "QML (quadratic minor lamination)",
    defn: "Thurston's lamination model of the Mandelbrot set — the parameter-plane analogue of the dynamical lamination. The two parameter rays landing at a hyperbolic component's root bound its wake; their chord is the component's minor leaf, and the QML is the collection of all minor leaves in the unit disk of external angles. Its defining property: every minor leaf spans a shorter arc ≤ 1/3, the widest being the period-2 root −3/4 ← {1/3, 2/3}; the period-3 roots give {1/7, 2/7} (the 1/3-bulb), {3/7, 4/7} (the airplane), {5/7, 6/7} (the 2/3-bulb). Shown as a corner disk widget on the parameter plane. Like the dynamical lamination it is measured, not assumed — the app lands the periodic parameter rays with the shipped Newton tracer (roots resolved by the parabolic-root Newton) and joins the ones that co-land, a faithful finite-detail approximation. z²+c.",
  },
  {
