...
// Two files: degrees 2–7 (what Tier 1 needs, in the main bundle) and degrees 8–15 (Tier 2's statistics,
// three quarters of the bytes, loaded by the worker and fetched lazily elsewhere).
const header = {
  source:
...
for (const [file, lo, hi] of [
  ["transitive.json", 2, 7],
  ["transitive-8-15.json", 8, 15],
]) {
  const target = resolve(here, `../src/engine/galois/data/${file}`);
  const part = Object.fromEntries(Object.entries(degrees).filter(([n]) => Number(n) >= lo && Number(n) <= hi));
  writeFileSync(target, `${JSON.stringify({ ...header, degrees: part })}\n`);
