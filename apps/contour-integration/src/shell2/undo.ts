...
/**
 * A structural comparison cheap enough to run on every commit.
 *
 * **`JSON.stringify` is safe for these fields and would not be for others**, so: they are plain
 * data, built by spreading and never cyclic, and they carry no functions — `Contour` has no sampled
 * representation and no closures (`engine/contour/model.ts` says why), and a `PathFn` or a compiled
 * evaluator lives on the RESOLUTION, never on the state. A field holding one would compare equal to
 * any other, because `JSON.stringify` drops a function silently.
 *
 ...
 * different. That can only ever produce an extra undo entry, never swallow a real one, which is the
 * direction a wrong answer here may err in.
 */
function stable(v: unknown): string {
  // `?? ""` is for the type: `JSON.stringify` returns `undefined` only for `undefined` itself, and
  // `same` has already established that both sides are non-null objects.
