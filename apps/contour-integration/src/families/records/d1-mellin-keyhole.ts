  ...
  golden: [
    {
      ...
      verifiedTo: 7e-16,
      method:
        "double-exponential (exp-sinh) quadrature on $(0,\\infty)$, which absorbs the $x^{\\alpha-1}$ endpoint singularity; all five points agree with $\\pi/\\sin(\\pi\\alpha)$ to $7\\times10^{-16}$ relative or better. The contour bookkeeping was verified separately: the closed-contour total is $2\\pi i\\,e^{i\\pi(\\alpha-1)}$ to $8.7\\times10^{-16}$ at $\\varepsilon = 10^{-9}$ and $R = 10^{9}$, and the edge factor comes back as $-e^{2\\pi i\\alpha}$ to the same truncation as the edge itself — the inner circle is still $1.1\\times10^{-2}$ there, since it vanishes only like $\\varepsilon^{0.3}$, which is a live demonstration of a slow limit",
    },
    {
      params: { alpha: 0.5 },
      value: "pi",
      ...
      verifiedTo: 2e-16,
      method: "exp-sinh double-exponential quadrature on $(0,\\infty)$; $\\sin(\\pi/2) = 1$, so the sine leaves the answer",
    },
    {
      params: { alpha: 0.75 },
      value: "pi/sin(3*pi/4)",
      ...
      verifiedTo: 1e-16,
      method: "exp-sinh double-exponential quadrature on $(0,\\infty)$, at the midpoint of the admissible strip",
    },
    {
      params: { alpha: 0.1 },
      value: "pi/sin(pi/10)",
      ...
      verifiedTo: 4e-16,
      method: "exp-sinh double-exponential quadrature on $(0,\\infty)$, near the edge of the strip, where the inner circle vanishes slowest",
    },
    {
      params: { alpha: 0.9 },
      ...
      verifiedTo: 7e-16,
      method: "exp-sinh double-exponential quadrature on $(0,\\infty)$; the reflection partner of $\\alpha = 0.1$, and equal to it to $4\\times10^{-16}$",
    },
  ],
};
