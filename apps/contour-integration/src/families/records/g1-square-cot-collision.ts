// SERIES-COT-COLLISION — `Σ_{n≥1} 1/n² = π²/6: the π cot(πz) kernel, where its pole and f's COLLIDE`, transcribed from
// `docs/contour-integration/gallery/tier-efg.md` §8.
//
...
// argument: `pi*cot(pi*z)/z^2` is meromorphic at 0 with a pole of order **1 + 2 = 3**, orders ADD,
...
//
// **The merged residue is the whole difference between this record and its companion.** `π cot(πz) = 1/z − (π²/3)z − (π⁴/45)z³ − …`,
// so the `−(π²/3)z` term multiplied by `1/z²` is what lands on `z⁻¹`: `Res₀ = -pi^2/3`. It comes from
...
// `π²/3` instead of `pi^2/6`, a factor-of-two error that looks entirely plausible, which is
...
export const g1SquareCotCollision: Family = {
  ...
  auxiliary: {
    integrand: "pi*cot(pi*z)/z^2",
    relation: "Re",
    note:
      "the summand is not integrated at all: the kernel's residue at every integer IS the summand, " +
      "and at n = 0 the two poles merge into one of order 3 whose residue is -pi^2/3",
  },
  ...
  closedForm: { expr: "-Res(pi*cot(pi*z)/z^2, 0) / 2", simplified: "pi^2/6" },
  ...
  traps: [
    ...
    },
    {
      ...
      message:
        "At a collision the residue does NOT factor. Res(K,0)·f(0) is meaningless — f(0) is infinite. The merged residue -pi^2/3 comes from the z⁻¹ coefficient of the PRODUCT's Laurent series: π cot(πz) = 1/z − (π²/3)z − (π⁴/45)z³ − …, so the −(π²/3)z term multiplied by 1/z² is what lands on z⁻¹. It comes from the kernel's Taylor TAIL, not from its pole.",
    },
    {
    ...
    },
    {
      ...
      message:
        "The kernel poles at n = ±1…±N contribute Σ_{n≠0} = 2·Σ_{n≥1}, so the target enters the residue sum with weight 2. Forgetting it reports π²/3 instead of pi^2/6 — a factor-of-two error that looks entirely plausible, which is precisely why the weight is a value the solve DERIVES from the target's range and checks.",
    },
  ],
