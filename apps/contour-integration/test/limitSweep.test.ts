...
function spyActions(): ShellActions & { calls: string[] } {
  ...
  return {
    ...
    setPieceRole: (id: string, role: string, lemma?: string) => push(`role:${id}:${role}:${lemma ?? "-"}`),
    renamePiece: (id: string, name: string) => push(`rename:${id}:${name}`),
    deletePiece: (id: string) => push(`delete:${id}`),
    insertPiece: (afterId: string, kind: string) => push(`insert:${afterId}:${kind}`),
    movePiece: (id: string, by: number) => push(`move:${id}:${by}`),
    reversePiece: (id: string) => push(`reversePiece:${id}`),
    setRenaming: (id: string | null) => push(`renaming:${id ?? "-"}`),
