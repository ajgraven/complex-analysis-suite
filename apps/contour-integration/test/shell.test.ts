...
describe("the argument window", () => {
  ...
  it("moves the cut, so LEGALITY refuses and no ∮ is printed", () => {
    ...
    expect(rail).toContain("Does the argument close?The argument is incomplete: the residue theorem does not apply.Hypotheses✓the contour is closed (orientation as drawn)Hypotheses✓no singularity lies on the contour (nearest distance 0.850)Hypotheses✓the branch cuts make the integrand single-valued off them — every cut reaches infinity, so no bounded component can carry monodromyHypotheses✗the R → ∞ circle crosses the cut Γ\\GammaΓ with no side assignedAssign the piece to the upper or lower side of the cut, or move the cut. Derivation — where it stops: LEGALITYThe problemThe integral to be evaluated, the integrand on the contour, and how the two are related.integrand1/(1+z)contourthe upper edge of the cut, the R → ∞ circle, the lower edge of the cut, the ε → 0 circleHypothesesThe contour is closed and meets no singularity; with a branch cut, the integrand is single-valued along it.= the contour is closed (orientation as drawn)endpoint-to-endpoint check over the piece list= no singularity lies on the contour (nearest distance 0.850)distance from each pole to each piece= the branch cuts make the integrand single-valued off them — every cut reaches infinity, so no bounded component can carry monodromyresearch 06 §2.1, decided over ℚ: every genuine branch point is on the forest, every bounded component sums to an integer, and no log is boundedaudit trail (1 step)✓ 1 branch point(s), 1 cut(s), no cycle⚠ the R → ∞ circle crosses the cut Γ\\GammaΓ with no side assignedthe R → ∞ circle crosses the cut 'Γ1'; the ε → 0 circle crosses the cut 'Γ1'✗ crossing 'Γ1' multiplies the integrand by e^(2πi·1/2) = −1Assign the piece to the upper or lower side of the cut, or move the cut.ResiduesThe singularities, their winding numbers $\\operatorname{Ind}_\\gamma(a)$, and their residues.per pole — the winding number and the count are separate facts:-1 + 0iorder 1n(γ) = 1Res = 1SolutionThe residue theorem, solved for the target.the residue theorem∮ f dz = 2πi Σₖ n(γ,aₖ)·Res(f,aₖ)independent cross-checkthe quadrature DISAGREES by 8.44e+0 — one of the two is wrong, so no value is reported⚠ ∮ f dz = 2πthe argument is DECIDED: a rational multiple of π is guessed numerically and then verified in exact arithmetic · the branch point carries no residue; each pole of the rational cofactor contributes z₀^α·Res(R, z₀) in the declared determination · the residue theorem gives 2π but the quadrature gives a value 8.44e+0 away, which is beyond its own error estimate — one of them is wrongarg z ∈ [-1/1·π, 1/1·π)✗ no quadrature cross-check: sampling z^α needs a determination, and a naive evaluator would use the principal branch and answer a different questionaudit trail (3 steps)✓ arg(−1) = -1/1·π, verified exactly over ℚ(i)(√d) by dividing z₀ by e^{irπ} and finding a positive real of modulus 1✓ the determination is an INPUT to the answer: the principal branch would change it by e^{2πiα} with nothing to warn you✓ the branch point is NOT a pole: there is no Laurent series at it and no residue to takeConclusionThe rigor of the conclusion is the weakest of its steps: $=$ exact, $\\le$ rigorous bound, $\\approx$ numerical.⚠ this argument does not closethe meet over 5 certificates — the weakest step in the argument, not the label of the answer✗ crossing 'Γ1' multiplies the integrand by e^(2πi·1/2) = −1 ∮ f(z) dz⚠ Refusedthe R → ∞ circle crosses the cut $\\Gamma$ with no side assignedAssign the piece to the upper or lower side of the cut, or move the cut. Contourcirclesemicircle ↑semicircle ↓indented semicirclerectanglestrip (2π)wedge (2π/3)square (N+½)keyholedogboneDraw a contourR = 4eps = 0.15the upper edge of the cuttarget1.75012377 + 7.3764e-31ithe R → ∞ circlevanishing piece0.22642109 + 5.80224159ithe lower edge of the cutmultiple of the target-1.75012377 + 7.3764e-31ithe ε → 0 circlevanishing piece0.01341986 + 0.08453941i Branch cutsmodulus contours|f| does not depend on the determination, so these contours run straight through any cut — which is the clearest evidence that a seam in the colour is a choice about the argument and not something the function does.Crossing a cutΓ1× e^(2πi·1/2) = −1shadow cuts+ branch pointthe colouring is built from the factorisation you declared, each factor in its own window — so the seam IS your cut, as it is under a gallery record. Moving the cut still changes the verdict and not the seam, because ∮ reads the window and never the geometry.Declared factorwindow:arg ∈ [0, 2π)arg ∈ [−π, π)written:(z − b)sheet:sheet 0 — the determination as declaredundeclareputs the whole integrand back in the box.⚠ the declared factor times R(z) is NOT the expression that was in the box: they differ by 2.01e+0 relative, at points where the two determinations agree and so ought to be identical. Either the branch factor is still inside R(z), or the exponent, orientation or constant does not match it= every cut reaches infinity, so no bounded component can carry monodromyz = 0.00 + 0.00i√ (α = 1/2)1/√ (α = −1/2)α = 1/3α = 2/3α = 1/4α = 3/4logremove Polesof the cofactor R(z) — the branch point carries no residue of its own.= exact-1 + 0i Res = 1Σ Res = 1 (exact, over every pole)");
