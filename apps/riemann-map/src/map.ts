...
// work (orbits, curve pushforward, invariants, tests) and a GLSL body for the per-pixel shader. This
// module is the single seam between "a string the user typed" and "something we can evaluate and
// render". Pure and DOM-free, so it unit-tests directly.
...
/** A compiled map: the JS evaluator, the GLSL `cvec fFn(cvec z, cvec c)` body, and a KaTeX string. */
...
export function compileMap(state: MapState): MapResult {
  ...
  try {
    ...
    return { ok: true, map: { jsFn, glslBody, latex } };
  } catch (e) {
    return { ok: false, error: e instanceof Error ? e.message : String(e) };
  }
}
