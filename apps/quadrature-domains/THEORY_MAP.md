...
| --- | --- | --- |
| `QD.Faber.inverseFaberAtPole(residues, phiTilde)` | `solver-faber.mjs:60` | Per-pole inverse Faber transform yielding A_{j,k} |
| `QD.Faber.inverseFaberAtInfinity(polyPart, f, c)` | `solver-faber.mjs:109` | Inverse Faber at ∞ for h's polynomial part (unbounded families) |
...
| Symbol | Where | Maps to |
| --- | --- | --- |
| `QD.FaberAnalysis.faberPolynomials(phi, N)` | `faber-analysis.mjs` | F₀..F_N coefficient lists from the φ-Laurent recurrence |
| `QD.FaberAnalysis.polynomialRoots(coeffs, opts)` | `faber-analysis.mjs` | Complex polynomial roots (Durand–Kerner + Newton polish) |
| `QD.FaberAnalysis.faberConvergence(phi, N)` | `faber-analysis.mjs` | Per-order roots + convergence flag (high-degree conditioning) |
...
| Block | Symbol | Where |
| --- | --- | --- |
| Compute (★) RHS (the target A) | `computeTargetA_QD` | `solver-qd.mjs:93` |
| Build φ evaluation | `evalPhi_QD` | `solver-qd.mjs:36` |
| ψ̃ Taylor series | `phiTaylorAt_QD` | `solver-qd.mjs:57` |
| Full residual (block (★) ⊕ block (●)) | `residual_QD` | `solver-qd.mjs:114` |
| Pack/unpack to flat real vector | `packPhi_QD`, `unpackPhi_QD` | `solver-qd.mjs:142`, `:149` |
| Initial guess (disk seed) | `diskInitialGuess_QD` | `solver-qd.mjs:189` |
| Continuation along a_j(t) | `continuationSolve_QD` | `solver-qd.mjs:244` |
| Identity verifier (∂Ω test) | `verifyQuadratureIdentity_QD` | `solver-qd.mjs:322` |
| Register on registry | `QD.registerFamily('boundedQD')` | `solver-qd.mjs:427` |

The same shape repeats for every other family (`solver-uqd.mjs`,
`solver-lqd*.mjs`, `solver-uqd-lqd*.mjs`) — see
...
criteria in `univalence.mjs` and the known `φ=z+z²` boundary self-crossing.
...
| Symbol | Where | Role |
| --- | --- | --- |
| `blaschkeEval(z, z0)` | `solver-lqd-common.mjs:52` | Standard Blaschke factor `(z − z_0) / (1 − conj(z_0) z)` |
| `blaschkeTaylor(zc, z0, L)` | `solver-lqd-common.mjs:65` | Taylor expansion of `b_{z_0}` at `zc` (length L+1) |
| `rHashLaurentAtInfinity(phi, L)` | `solver-lqd-common.mjs:132` | Laurent of `r#(z)` at z = ∞ |
| `blaschkeLaurentAtInfinity(z0, L)` | `solver-lqd-common.mjs:170` | Laurent of `ln(b_{z_0}(z))` at ∞ |
| `phiLaurentAtInfinity_UQDL(phi, L)` | `solver-lqd-common.mjs:204` | φ-Laurent at ∞ for unbounded non-singular LQD (HANDOFF #21) |
| `phiLaurentAtInfinity_UQDLS(phi, L)` | `solver-lqd-common.mjs:237` | φ-Laurent at ∞ for unbounded singular LQD (HANDOFF #22) |
...
| Block | Symbol | Where |
| --- | --- | --- |
| (★) target | `computeTargetA_LQDS` | `solver-lqd-singular.mjs:137` |
| φ evaluation | `evalPhi_LQDS` | `solver-lqd-singular.mjs:99` |
| ψ̃ Taylor | `phiTaylorAt_LQDS` | `solver-lqd-singular.mjs:109` |
| Residual (★ ⊕ ● ⊕ q-equation at 0) | `residual_LQDS` | `solver-lqd-singular.mjs:151` |
| Schema (pack/unpack via `packPhiBySchema`) | `SCHEMA_LQDS` | `solver-lqd-singular.mjs:273` |
| Initial guess | `initialGuess_LQDS` | `solver-lqd-singular.mjs:299` |
| Diverse seeds (Blaschke z₀ multistart) | `diverseInitialGuess_LQDS` | `solver-lqd-singular.mjs:463` |
| Identity verifier | `verifyQuadratureIdentity_LQDS` | `solver-lqd-singular.mjs:549` |
| Register on registry | `QD.registerFamily('boundedLQD_singular')` | `solver-lqd-singular.mjs:629` |
...
| --- | --- | --- |
| β-correction (polynomial-at-∞ part) | `phi.lqdBeta` | populated in `solver-uqd-lqd.mjs`, `solver-uqd-lqd-singular.mjs`; carried through `clonePhi` everywhere |
| γ synthetic-branch at z₀ | `phi.lqdGamma` | `solver-uqd-lqd-singular.mjs`; merged into branches via `_phiWithSyntheticBranch` |
| (★)_F equations match β to h's poly-at-∞ | via `inverseFaberAtInfinity` + `phiLaurentAtInfinity_UQDL` | `solver-lqd-common.mjs:204` |
| (★)_Γ block (γ matches principal at w=0) | uses `inverseFaberAtPole` directly | `solver-uqd-lqd-singular.mjs` |
| Schwarz GPU support | `u_lqdBeta` uniforms, γ-merged branches | `schwarz/schwarz-webgl.mjs` |
...
**Code.** [`app/direct/direct-common.mjs`](app/direct/direct-common.mjs) —
...
**Code.** [`app/schwarz/schwarz-common.mjs`](app/schwarz/schwarz-common.mjs) —
...
| --- | --- | --- |
| Householder QR (real, m ≥ n) | `houseQR` — `solver.mjs:195` | P1.2. Backward-stable; replaces Gauss-Jordan on the normal equations. Surfaces `condEst`. |
| Square linear solve | `solveLinearSystem` — `solver.mjs` (post-QR) | Now routes through `houseQR.applyQt` + `backSolve`. |
| Least-squares solve | `solveLeastSquares` — `solver.mjs` (post-QR) | Direct QR; no `A^T A` formation. |
| Newton with Armijo + deflation | `newtonSolve` — `solver.mjs` | Damped Newton, finite-diff Jacobian (auto forward→central when ill-conditioned, #11), bounded iterative refinement, optional pluggable analytic Jacobian. |
| Top-level inverse solver | `solveInverseQD` — `solver.mjs:790` | Stages A1-A5 (direct / continuation / multistart / diverse seeds / deflation). |
| Boundary sampler (adaptive) | `sampleBoundaryAdaptive` — `solver.mjs` | Used everywhere φ(∂𝔻) is needed. |
| Univalence check | `isBoundaryUnivalent` — `solver.mjs` | Polygon self-intersection on sampled boundary. |
| Identity test points | `chooseHoleTestPoints` — `solver.mjs` | Picks interior points in the hole (ray-cast) ranked by clearance from ∂Ω + poles; shared by the unbounded identity verifiers. |
| Max conformal radius c\* | `estimateMaxConformalRadius` — `solver-cmax.mjs` | Bracket+bisection with a two-regime gate: genuine-QD (univalent + identity) away from the cusp, cusp criterion `g = max|z|` over φ′ zeros (`< 1`) near it. Reports `mechanism` (cusp/fold) and `confidence ∈ [0,1]` (#11). Warm-start gauge injection + confirm-invalid guard; dependency-injected `solveFn` (worker in browser, sync solver in tests). |
| Near-cusp identity resolution | `verifyQuadratureIdentity_UQD` — `solver-uqd.mjs` | Escalates the uniform-θ node count (doubling to a cap) when the near-cusp gate fires (`min/mean |φ′| < 0.08`) and the error is still converging, so a genuine QD near c\* is not mis-rejected. Uniform spectral trapezoid is kept away from cusps (grading would hurt it); `adaptiveSamples:false` forces single-pass (#11). |
| Boundary observables | `boundaryObservables` / `harmonicMeasure` / `estimateAccuracy` — `observables.mjs` (page-only) | From a solved φ via one φ/φ′/φ″ sweep (`phiTaylorAt`): signed curvature κ(θ)=Im(conj γ′·γ″)/|γ′|³ (κ→∞ at cusps), area (shoelace) + perimeter + centroid + complex area moments `M_k=∬_Ω w^k dA` (Stokes); harmonic-measure density `ρ=1/(2π|φ′|)`; multi-resolution identity error → significant-digits + condition estimate. Foundational primitives for the dynamics features. |
| Symmetry detector | `detectSymmetry` — `symmetry.mjs` (page-only) | Domain symmetry group from φ via the Riemann-map **intertwining** φ(e^{2πi/n}z)=c+e^{2πi/n}(φ(z)−c): EXACT index-shift / reflection tests on M=2520 boundary samples ⇒ `{rotationalOrder, reflectionAxes, center, continuous}`. Robust for non-star-shaped ∂Ω. Drives the #9 symmetry-axis overlay; partial TODO #11 (#9). |
| Analytic oracles | `ThesisExamples` / `checkOracle` — `thesis-examples.mjs` (page-only) | Curated canonical QDs each with closed-form expectations; `checkOracle(phi,hData,oracle)` routes each field to the matching detector (observables / cusps / symmetry / accuracy / c\*) and grades pass/warn/fail. Powers the #8 example gallery + oracle card and a regression test. |
