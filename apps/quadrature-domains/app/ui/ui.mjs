...
// LQD_PRESETS_UNBOUNDED_SINGULAR) AND mirrors them under QD_UI.Presets.*
...
// is also exposed via QD_UI.state for explicit-namespace callers.
...
const sub = n => QD.Format.subscript(n);
...
function escapeHTML(s) {
  return (window.QD && QD.QoL && QD.QoL.escapeHTML)
    ? QD.QoL.escapeHTML(s)
...
const DomainPlot = QD_UI.installDomainPlot({ state, modeDescriptor, formatTick, sub });
...
if (QD_UI && QD_UI.installThesis) QD_UI.installThesis(uiCtx);
...
if (QD_UI && QD_UI.installFaber) QD_UI.installFaber(uiCtx);
...
QD.Direct._setPlotBoundary = function (boundaryPts, opts) {
...
QD.Direct._setPlotOverlay = function (overlayBoundary) {
...
//
// We expose these on QD_UI so param-slice-ui.js can find them
// without having to be loaded after this file.
// =============================================================================

QD_UI.snapshotScenario = function () {
  const built = buildHData();
...
QD_UI.loadScenarioIntoQdTab = function (scenario, mode) {
