...
(function (global) {
  ...
  const S = {
    ...
    tooltips: {
      ...
      groebner: `Compute a Gröbner basis of the selected equality nodes (or all of them) — the multivariate generalization of the resultant — in the monomial order set under Advanced. To remove variables instead, use "Eliminate picked variables" above. Buchberger over ℚ(i) with the Gebauer–Möller criteria + sugar selection; runs off the main thread (cancellable). A cost blow-up suggests assuming variables real or using the CAS export.`,
