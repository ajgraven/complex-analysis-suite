 ...
 * image renderers but read by metadata viewers and by {@link readPngText} here — so an exported figure
 * carries the exact share-link that produced it (catalog K3).
 *
 * Pure and dependency-free (no DOM), so it is unit-tested directly and can run anywhere. Ported from the
 * Complex-Dynamics app's `render/pngMetadata.ts` (apps can't import each other; extract downward only on
 * a proven second consumer, ADR-0007 — this deliberate copy is that second consumer, and the seam to
 * watch).
 */
...
export function readPngText(png: Uint8Array): Record<string, string> {
  ...
  while (pos + 8 <= png.length) {
    ...
    if (type === "tEXt") {
      ...
      out[keyword] = String.fromCharCode(
        ...data.subarray(Math.min(sep + 1, data.length)),
      );
    }
    pos += 12 + len;
  }
