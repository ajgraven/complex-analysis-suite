...
| Certified univalence filter: reconstruct each real solution's φ and test schlichtness (φ′≠0 in 𝔻 via critical points; simple boundary φ(∂𝔻)) → # GENUINE quadrature domains vs algebraic real solutions | Algebra "Certify univalence" (`doCertifyUnivalence` → `QD.findCriticalPoints` + `QD.isBoundaryUnivalent` on the reconstructed φ) | [`app/algebra/algebra-ui.js`](app/algebra/algebra-ui.js) |
