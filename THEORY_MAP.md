...
| Audit-trail reductions (each appends a labeled column; column 0 = original): fix a value (exact ℚ(i)) + auto-propagate the linear cascade, fix φ(0)=w₀ | `AlgebraStore.substituteValue` / `reducePropagate` / `fixW0` | [`app/algebra/algebra-store.js`](app/algebra/algebra-store.js) |
