...
// =============================================================================
// Asset-manifest anti-drift battery (Phase-1 release hardening). Guards the
// single-source-of-truth invariants the index.html loader + SW + cache-version
// generator depend on. Self-contained: evaluates asset-manifest.js in a vm and
// reads index.html.
// =============================================================================
(function () {
  try {
    const fsM = require('fs'), pathM = require('path'), vmM = require('vm');
    const appDir = __dirname;
    const sandbox = { self: {} };
    vmM.createContext(sandbox);
    vmM.runInContext(fsM.readFileSync(pathM.join(appDir, 'asset-manifest.js'), 'utf8'),
    ...
    // Regression for the SW-precache omissions this work fixed.
    ...
    const missing = M.PAGE_SCRIPTS.filter(f => !fsM.existsSync(pathM.join(appDir, f)));
    ...
    const html = fsM.readFileSync(pathM.join(appDir, 'index.html'), 'utf8');
    ...
    try {
      cp.execFileSync('node', [pathM.join(appDir, '..', 'scripts', 'gen-cache-version.js'), '--check'],
                      { stdio: 'pipe' });
