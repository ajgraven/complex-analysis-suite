...
| Item | Axis | Effort | Note |
|---|---|---|---|
| **Polynomial matings on the live sphere** ✅ **engine+render+general shipped** (#249–252) | math | **XL** | THE marquee research feature — computes *and* renders trustworthy matings for *any hyperbolic p/q-bulb ⊔ basilica*. Took the **marked-point Thurston pullback** (Jung), not Medusa/raw Thurston; a conjugation-symmetry gate refuses wrong-basin captures. Tail = general 2nd parent + full slow-mating homotopy. Reuses the sphere mode. *(Pending task #73 "Schwarz/deltoid" folds in here — it's a degree-2 antiholomorphic mating, not a preset.)* |
