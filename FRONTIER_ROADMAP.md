...
- ✅ **Renormalization — internal-address navigator** (§4 **B7** internal-address half + **B10** address label) `[math/teaching]` (2026-07-05) — the combinatorial "GPS" of a hyperbolic component: its **internal address**, the strictly increasing renormalization periods 1 = S₀ < … < Sₖ = period (Lau–Schleicher). What distinguishes components of the *same* period — rabbit (period 3, satellite) = 1-3 vs airplane (period 3, primitive) = 1-2-3. Exact integer arithmetic on the doubling map (a rigorous combinatorial fact). **The engine already existed in `src/combinatorics/`** (the forward `stripExternalAddress` address→angles chain); the new work is the **inverse** `internalAddressFromAngle` (kneading via the {θ/2,(θ+1)/2} partition → the Lau–Schleicher ρ-function) plus two UI access points. **#257** first added a duplicate module → **#258** consolidated it into `src/combinatorics/stripping.ts` (one source of truth, + a forward↔inverse round-trip test). **#259** made the Symbolic console a **two-way dictionary** (enter an angle → read off the address; fills the address box to strip it back). **#260** taught the **"Angles of a point"** finder to append the clicked component's address (click ∂M → its address; a Misiurewicz point honestly reports "no address"). Oracles: rabbit 1/7→1-3, airplane 3/7→1-2-3, the period-4 census, and the documented Schleicher **22/127 → 1-3-5-7** (test 669 → 684). **Deferred:** the Hubbard-tree drawing (rest of B7) and the small-M-copy tuning marker (rest of B10 — an exact baby-M outline is research-grade).
...
| **Renormalization / tuning navigator** ◧ **internal-address label shipped** (#257–260) | math | **L marker / XL straightening** | "Mandelbrot in Mandelbrot." The **internal-address label is shipped** (engine + angle→address console + click→address finder — rabbit 1-3 vs airplane 1-2-3); the small-M-copy tuning marker + qc straightening remain deferred. |
...
- **B7 · Hubbard trees + kneading/internal-address** — `[math]` **M–L**. ◧ **Internal-address half SHIPPED (#257–260):** kneading from the doubling itinerary vs the {θ/2,(θ+1)/2} partition + the internal address by Schleicher's ρ-function (`internalAddressFromAngle` in `src/combinatorics/stripping.ts`, the inverse of the shipped forward stripper), surfaced both as an angle→address console (#259) and by clicking a component (#260). **Oracle verified in tests: θ=22/127 → 1-3-5-7**, plus rabbit 1/7→1-3 vs airplane 3/7→1-2-3. **Deferred:** the Hubbard-*tree* drawing (abstract tree = postcritical points joined by regulated arcs, q-star bulb rule with rotation p/q; rabbit tree = tripod). Sources: Schleicher arXiv math/9411238, Orsay Notes Ch.4, Bruin–Schleicher.
...
- **B9 · Polynomial matings on the sphere** — `[math]` **XL**. ✅ **Engine Stages 1–2 SHIPPED** (#249 pullback core, #250 render) via the **marked-point Thurston pullback** (Jung arXiv:1706.04177 — iterate the *formal*-mating pullback; the postcritical points collide but the rational maps converge to R), NOT Medusa's rectify/prune. `matingEngine.ts` mates any PCF quadratic with the **basilica** → g(z)=(z²−x₁)/(z²−1); verified z²+i ⊔ basilica = exactly (z²+2)/(z²−1), rabbit/corabbit → (z² − e^{±2πi/3})/(z²−1), basilica ⊔ basilica refused (obstructed); rendered on the live sphere in Marty (spherical-derivative) mode. **Stage 3 SHIPPED (#252):** a **conjugation-symmetry gate** (x₁(c̄)=conj(x₁(c))) generalises it to *any hyperbolic p/q-bulb ⊔ basilica*, trustworthily — refuses a wrong-basin capture (the airplane) rather than drawing it (`bulbCenter`/`mateBulbWithBasilica` + a "Render p/q ⊔ basilica" input). **Stage 4 — general second parent SHIPPED (#254 engine, #255 UI):** mate *two arbitrary hyperbolic bulbs* p₁/q₁ ⊔ p₂/q₂ via the **Boyd–Henriksen F_{u,v}** normal form (both critical values free), obstruction gated by **Tan Lei** (refuse conjugate limbs) and basin picked by **swap-consistency** (order-independent up to z↦1/z ⇒ diagonal u·v=1); verified against the shipped ⊔basilica maps via the Milnor multiplier invariant. **Tail:** a Misiurewicz second parent; the full **slow-mating homotopy** (R_t = exp(2^{1−t}), would extend trustworthiness to Misiurewicz/edge cases); then full Medusa (specialist tail). Sources: Milnor mate03, Boyd–Henriksen Medusa (arXiv 1102.5047), Jung (arXiv 1706.04177), Chéritat qmate.
- **B10 · Renormalization / tuning navigator** — `[math]` **L marker / XL straightening**. ◧ **Internal-address label SHIPPED (#257–260)** (see the Shipped bullet + B7). **Deferred:** tuning `c=c₀∗c_h` via internal/external-angle correspondence + the small-M-copy marker (an exact baby-M outline is research-grade — the tuning map isn't affine); qc straightening. **Oracle:** Feigenbaum c=−1.401155, δ=4.6692; −1∗−1=−1.310702. Sources: McMullen §7.4, Jung embed.pdf.
