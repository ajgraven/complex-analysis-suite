...
| Item | Axis | Effort | Note |
|---|---|---|---|
| **Polynomial matings on the live sphere** ✅ **engine+render shipped** (#249–250) | math | **XL** | THE marquee research feature — computes *and* renders verified matings. Took the **marked-point Thurston pullback** (Jung), not Medusa/raw Thurston; curated ⊔ basilica so far, Stage 3 = slow-mating init for arbitrary pairs. Reuses the sphere mode. *(Pending task #73 "Schwarz/deltoid" folds in here — it's a degree-2 antiholomorphic mating, not a preset.)* |
...
external-ray landing (θ→point)  ──┬─→ angles-of-a-point / biaccessibility
   [Tier 1, math]                 ├─→ parameter wakes & limbs
                                  ...
                                  └─→ de-risks spider ─→ matings ✅ engine+render shipped (#249–250)
