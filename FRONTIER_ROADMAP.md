...
- ✅ **Yoccoz puzzles** (§4 **B5**) `[math]`, **Stage 1** — the first downstream unlock of external-ray landing: the depth-_n_ puzzle graph on the dynamical plane (the α orbit-portrait rays pulled back n times, Θₙ = {θ : 2ⁿθ ∈ A}, q·2ⁿ rays in violet), with a depth slider and a repelling-α gate (**#227**). *Deferred:* Stage 2 (the critical-piece nest + itinerary labels), Stage 3 (parapuzzle + tableau), and the equipotential piece-caps.
...
| **GPU BLA traversal** | depth | **M–L** | 3–1000× fewer iterations → interactive deep zoom. `bla.ts` table already built + tested (#155); wire it into the perturbation kernel as a float texture. |
...
|---|---|---|---|
| **General-f perturbation** (poly → rational) | depth | **M–L** | Makes the headline "hybrid f" deep-zoomable. Shares `A₁=f′(Z), B₁=∂f/∂c` with BLA — **build jointly**. |
...
| **Lavaurs laminations / pinched-disk + Hubbard trees + kneading/internal-address** | math | **M** | Exact integer combinatorics; complements orbit portraits. |
...
| Item | Axis | Effort | Note |
|---|---|---|---|
| **Polynomial matings on the live sphere** | math | **XL** | THE marquee research feature. Start from the **Medusa algorithm**, not raw Thurston. Reuses the sphere mode. *(Pending task #73 "Schwarz/deltoid" folds in here — it's a degree-2 antiholomorphic mating, not a preset.)* |
...
external-ray landing (θ→point)  ──┬─→ angles-of-a-point / biaccessibility
   [Tier 1, math]                 ├─→ parameter wakes & limbs
                                  ...
                                  └─→ de-risks spider ─→ matings (Medusa)
...
- **B6 · Lavaurs laminations / pinched-disk model** — `[math]` **M**. Angles `n/(2^p−1)`; pair smallest-not-yet-connected to next-smallest subject to no chord crossing. Pure exact integer arithmetic. **Oracle:** basilica leaf ⅓–⅔; rabbit ideal triangle {1/7,2/7,4/7}. Sources: Wikibooks Lavaurs, Thurston, arXiv 2101.08101.
...
- **B9 · Polynomial matings on the sphere** — `[math]` **XL**. Glue two PCF Julia sets along ray-equivalence → quadratic rational map; render on the existing live sphere. Use the **Medusa algorithm** (pixel-based) or slow-mating; detect Levy/Thurston obstruction; realizable when c₁,c₂ not in conjugate limbs. Sources: Milnor mate03, Boyd–Henriksen Medusa (arXiv 1102.5047), Chéritat qmate.
...
- **Perturbation is genuinely z²+c-only** — hardcoded in three places: the δ-recurrence (`shaderBuilder.ts:187`), the CPU reference (`perturbation.ts:52`), and the `probeMandelbrot()` eligibility gate (`glPlot.ts:709`). df64 (~4× shallow cost) is the only general-f deep-zoom path today.
- **`bla.ts` CPU table exists and self-validates; no GPU traversal** (the D2b item / A1 above).
