...
Polynomial Roots and Polynomial Root Analysis (ADR-0047, published at PRA-5; PRA-10's narrative layer landed, PRA-9 and
PRA-10's long tail to come), plus one built but not yet published — the anti-holomorphic Correspondences tool — riding
...
11. **Deployment:** each app builds static (`base: "./"`) so its assets resolve from any path.
    **`.github/workflows/deploy-pages.yml` publishes automatically on every push to `master`**
    ...
    `contour-integration/`, `polynomial-roots/`, and `polynomial-root-analysis/` beneath it.
    `apps/correspondences` is **built but not published** (the launcher shows it as "Coming soon").
    There are **two** workflows: `ci.yml` (jobs `build` + `browser` + the non-blocking
...
reads `= S₅`, x⁵ + 20x + 16 `= A₅`, the D₅ quintic "not yet identified" with no name. PRA-5 named every
...
invariant reads `= 40` (Dummit's θ), and a played motion says which invariants it moved. PRA-7 opened
families p(t, z) over ℚ: the branch points in t exact, a flower of straight-tethered lassos from a draggable
base point run through the certified tracker (now following a family — `@cas/monodromy`
...
3-cycle word; S₅'s derived series 120 → 60 → 60, enumerated. PRA-10's narrative layer (PRA-9 skipped
for now by the owner) added a ten-step tour of Arnold's proof whose predictions are graded from the
run, a front door of eleven classics on `@cas/ui`'s `createModal` (moved from Contour Integration, its
second app), and a faded drill — *which word rules this formula out?* — answered by the ladder's
theorem verdict, never a key; the long tail (Dummit, Tier 1 to 8–11, Schreier–Sims, PARI-wasm,
hand-offs) awaits the owner's order.
Read
