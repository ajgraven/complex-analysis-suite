...
export const GLOSSARY: GlossaryEntry[] = [
  ...
  },
  {
    id: "mating",
    term: "Mating",
    defn: "Glue two filled Julia sets along their boundaries (by the angle-doubling identification of external rays) to build a single rational map that combines both dynamics. Two post-critically-finite quadratics z²+c₁, z²+c₂ admit a (conformal) mating iff c₁ and c₂ are NOT in complex-conjugate limbs of the Mandelbrot set (Rees–Shishikura–Tan Lei). For main-cardioid bulbs, the p/q limb's conjugate is the (q−p)/q bulb, so the obstruction is p₁/q₁ + p₂/q₂ = 1; the 1/2 bulb is the only self-conjugate one, so every other bulb can be mated with itself. Beyond the mateability test, the app can also RENDER a mating: the mated rational map is computed by the marked-point Thurston pullback (Wolf Jung's algorithm — iterate the formal-mating pullback; the postcritical points collide but the rational maps converge), then drawn as a Julia set on the dynamical plane / Riemann sphere, where the two parent Julia sets appear on the two hemispheres. The second parent is the basilica (z²−1) so far: named examples (z²+i ⊔ basilica = (z²+2)/(z²−1); rabbit ⊔ basilica = (z² − e^{2πi/3})/(z²−1)) plus any satellite bulb p/q ⊔ basilica. That general path is made trustworthy by a conjugation-symmetry gate — a mating obeys x₁(c̄)=conj(x₁(c)), so the engine pulls back both c and c̄ and refuses a bulb whose result isn't self-consistent (rather than draw a wrong map). A general second parent and the full slow-mating homotopy are still in progress.",
  },
  {
    id: "misiurewicz",
