/**
 * External rays for the Mandelbrot set (parameter plane) and filled Julia sets
 * (dynamical plane), for z²+c. An external ray at angle θ (in turns) is the image
 * under the Böttcher map of the straight ray {r·e^{2πiθ} : r > 1}; as r → 1⁺ it lands
 * on the boundary at "external angle θ".
 *
 * Tracing uses the Böttcher approximation z_m ≈ Φ(·)^{2^m}: at depth m the target is the
 * fixed-modulus point R·exp(2πi·2^m·θ) (R the escape radius), and we Newton-solve the
 * m-th iterate for the unknown (c on the parameter plane, z₀ on the dynamical plane),
 * marching m outward and using each solved point as the next initial guess (continuation).
 * Fixing the target modulus at R avoids the w^{2^m} overflow.
 *
 * Pure (no DOM/GL), so it is unit-tested. Double-precision-bound: the angle doubling
 * 2^m·θ loses a bit per step, so landings deep in the set (large `depth`) are approximate —
 * deep-zoom rays would need extended precision (deferred).
 */
...
/**
 * Parameter-plane external ray of the Mandelbrot set at angle θ. Points run from the
 * far field inward to the landing point on ∂M. Newton variable = c; the m-th iterate of
 * the critical orbit z_m(c) (z₀ = 0) and its derivative dz_m/dc (recurrence d ← 2zd + 1).
 */
...
/**
 * Dynamical-plane external ray of the filled Julia set K_c at angle θ (c fixed). Newton
 * variable = z₀; the m-th iterate z_m (z₀ = the unknown) and its derivative dz_m/dz₀
 * (recurrence d ← 2zd, d₀ = 1).
 */
 ...
 * p/q satellite bulb of the main cardioid. Among the period-q cycles of the doubling map
 * x ↦ 2x (mod 2^q − 1), exactly one is order-isomorphic to rigid rotation by p/q; the two
 * angles bounding its smallest circular gap are the landing rays (1/2 → {1/3, 2/3},
 * 1/3 → {1/7, 2/7}, 2/3 → {5/7, 6/7}, 1/4 → {1/15, 2/15}). Returns null for q < 2 or a
 * non-reduced p/q.
 */
