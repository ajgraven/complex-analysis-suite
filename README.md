...
It currently hosts **fourteen** applications — thirteen published, Correspondences built but not yet published — riding **fourteen** shared `@cas/*` packages:
...
| **2D Electrostatics** (`apps/2d-electrostatics`)   | The complex potential `W = φ + iψ` as an interactive electrostatic field: drop/drag charges, sources, sinks, vortices, doublets; plus a conformal-transplant polygon page — flow past or inside a polygon via Schwarz–Christoffel | Vite + TypeScript |
...
| **Polynomial Root Analysis** (`apps/polynomial-root-analysis`) | *Published at PRA-5 (ADR-0047); PRA-6 added the Galois correspondence, PRA-7 families p(t, z), PRA-8 the Abel–Ruffini ladder, PRA-10 the tour, the front door of classics and the faded drill; PRA-9 and PRA-10's long tail to come.* One polynomial and what acts on its roots: roots and coefficients as two linked draggable point sets with exact Smith inclusion discs, the Galois group over ℚ in three honest tiers, loop monodromy by certified continuation, and Arnold's topological proof of Abel–Ruffini as commutator loops. PRA-1 (the two-pane sandbox: drag roots or coefficients over ℂ/ℝ/ℚ, every root in an exactly certified disc) and PRA-2 (critical points against Gauss–Lucas, the exact discriminant and branch points, certified pseudozero counts) and PRA-3 (certified loop monodromy: lassos, commutators, the group they generate, root-side motions, the braid) and PRA-4 (the Galois group over ℚ: exact factorisation, the cycle types the primes below 1000 prove, and `= Sₙ`/`= Aₙ` only when a theorem names it) and PRA-5 (every group of degree ≤ 7 named exactly by resolvents certified from the roots, as permutations of the reader's numbered roots that can be played as motions; degrees 8–15 ranked by the primes and labelled `≈`) have landed; see [`docs/polynomial-root-analysis/STATUS.md`](docs/polynomial-root-analysis/STATUS.md) | Vite + TypeScript |
...
`contour-integration/`, `polynomial-roots/`, and `polynomial-root-analysis/` beneath it. Correspondences is built but not yet
...
│   ├── exact/                ← @cas/exact       exact polynomial arithmetic, 𝔽ₚ[x] and factorisation over ℤ (CD, Correspondences, Polynomial Root Analysis)
...
└── apps/                     ← thin applications; each a Vite build that consumes packages
    ...
    └── polynomial-root-analysis/ ← one polynomial: roots ↔ coefficients, Galois group, monodromy, Abel–Ruffini
```
...
> exterior Faber-transform engine behind the Faber Transform and Potential Theory apps. The
> **twelfth** package, `@cas/flow`, is the conformal-transplant kernel carved out of 2D Electrostatics when that app
> split into three ([ADR-0036](docs/DECISIONS.md#adr-0036-split-2d-electrostatics-into-three-apps-extract-casflow)).
> The **thirteenth**, `@cas/rigor`, is the only one **created rather than extracted**
> ([ADR-0040](docs/DECISIONS.md)): the honest-labelling guardrail had no shared code at all, only
